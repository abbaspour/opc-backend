...
class Instance {
    constructor(readonly name: string | undefined, readonly status: ServiceStatus,
...
const getInstance = async (accountNo: number): Promise<Instance[] | undefined> => {
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
    ...
    if(!instance) return error('missing instance path parameter');

    console.log(`querying instance info: ${instance}`);

    const instances = await getInstance(accountNo);

    console.log('instances: '  + JSON.stringify(instances));

    return {
        statusCode: 200,
        body: JSON.stringify(instances ? instances[0] : null),
        headers: {
