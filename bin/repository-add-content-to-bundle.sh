...
function usage() {
    cat <<END >&2
USAGE: $0 [-e env] [-c sample-policy.rego] [-v|-h]
        -e file     # .env file location (default cwd)
        -b bundle   # bundle name. defaults to $DEFAULT_BUNDLE
        -c content  # policy or data file
        -h|?        # usage
        -v          # verbose
...
curl -X POST -H "Authorization: Bearer ${access_token}" "https://api.openpolicy.cloud/repository/v1/bundles/${bundle}/contents/${content}"
